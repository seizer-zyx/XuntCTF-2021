...
class TetrisElement {
  ...
  rotate(val) {
    //小方块旋转，因为还需要反向旋转，所以需要传个参数，如果不传，默认为1
    val = val ? val : 1;
...
function fixed() {
  ...
  try {
    for (var i = 0; i < dynamicElement.squareList.length; i++) {
      squareSet[dynamicElement.squareList[i].y][
        dynamicElement.squareList[i].x
      ] = dynamicElement.squareList[i];
    }
    dynamicElement = null;
...
function render(all) {
  ...
  if (dynamicElement) {
    ...
    for (var k = 0; k < dynamicElement.squareList.length; k++) {
      dynamicElement.squareList[k].style.left =
        30 * dynamicElement.squareList[k].x + "px";
      dynamicElement.squareList[k].style.top =
        30 * dynamicElement.squareList[k].y + "px";
    }
  }
  if (all) {
    //当传入true的时候，渲染全部小方块，不是每次都全部渲染以保证性能
    for (var i = 0; i < squareSet.length; i++) {
      for (var j = 0; j < squareSet[i].length; j++) {
...
function checkOutOfRange() {
  ...
  for (var i = 0; i < dynamicElement.squareList.length; i++) {
    if (
      ...
      max =
        dynamicElement.squareList[i].x < 0
          ? 0 - dynamicElement.squareList[i].x
          : 9 - dynamicElement.squareList[i].x;
    }
  }
...
function init() {
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.key == "ArrowUp") {
      ...
      if (checkCrash()) {
        //检查是否会碰撞，如果会碰撞，则状态返回，因为没有进行刷新，用户视觉无影响
